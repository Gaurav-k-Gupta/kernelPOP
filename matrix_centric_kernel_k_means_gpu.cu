...
// Computes E = K * V (Matrix-centric Sparse Distance calculation)
...
// Computes C = diag(V^T * E) (Matrix-centric Cluster Self-Distance)
...
int main(int argc, char** argv) {
    ...
    // cuBLAS GEMM Matrix Multiplication (As explicitly requested by the paper)
    ...
    // Kernel K-Means++ Initialization (Robust Symmetry Breaking)
    srand(42);
    ...
    free(min_dists); free(centers);

    int *h_count = (int*)malloc(K_CLUSTERS*sizeof(int));

    // Matrix-Centric Update Algorithm Loop
    ...
    cudaFree(d_P); cudaFree(d_B); cudaFree(d_K); cudaFree(d_E); cudaFree(d_C); cudaFree(d_cluster); cudaFree(d_count);
    return 0;
}